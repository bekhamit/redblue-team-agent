// harness-filesystem.ts - Test harness for Filesystem MCP

import { TestCase, TestResult, TestSummary, MCPResponse } from '../../shared/types.js';
import { validateFilesystemTestResult } from '../../shared/validators.js';
import { ProtectiveMCPWrapper, ProtectionRules } from '../../shared/protective-wrapper.js';

export class FilesystemMCPTestHarness {
  ...
  /**
   * Connect to the filesystem MCP server via Docker
   */
  ...
  /**
   * Run a single filesystem test case
   */
  private async runTest(testCase: TestCase): Promise<TestResult> {
    ...
    // Validate using filesystem validators
    const validation = validateFilesystemTestResult(testCase, response, executionTime);
  ...
  /**
   * Run multiple filesystem test cases
   */
  async runTests(testCases: TestCase[]): Promise<TestSummary> {
    console.log(`🧪 Running ${testCases.length} filesystem test cases...\n`);
  ...
  printComparison(before: TestSummary, after: TestSummary, initialRuleCount: number): void {
    ...
    console.log('\n🛡️  RESULT: Wrapper successfully learned from filesystem attacks!');
    console.log('   Filesystem MCP is now safe to use through protective middleware.\n');
  }
}
